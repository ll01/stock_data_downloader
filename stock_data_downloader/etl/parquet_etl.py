...
def load_stock_stats(
    ...
    try:
        ...
        if skipped > 0:
            logging.warning(
                f"Skipped {skipped} rows with missing data from {sim_data_file_path}"
            )
    except FileNotFoundError:
        logging.warning(
            f"Simulation data file not found: {sim_data_file_path}. Generating synthetic stats if archetype is provided."
        )
    ...
    if default_archetype:
        ...
        logging.info(
            f"Using default archetype '{default_archetype}' for missing ticker stats."
        )
