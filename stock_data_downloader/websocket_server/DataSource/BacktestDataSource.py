...
class BacktestDataSource(DataSourceInterface):
    def __init__(
        ...
        # If start_prices is not defined in backtest_config but ticker_configs have start_price,
        # populate start_prices with those values
        if not self.backtest_config.start_prices or len(self.backtest_config.start_prices) == 0:
            self.backtest_config.start_prices = {}
            for ticker, config in self.ticker_configs.items():
                if config.start_price is not None:
                    self.backtest_config.start_prices[ticker] = config.start_price
        ...
        self._randomize_counter = 0

        # Auto-generate delisted assets based on include_delisted_probability
        if self.backtest_config.include_delisted_probability > 0:
            rng = random.Random(self.backtest_config.seed)
            for ticker in list(self.ticker_configs.keys()):
                if rng.random() < self.backtest_config.include_delisted_probability:
                    # Generate a random timestep for delisting (1-based)
                    delist_step = rng.randint(1, self.backtest_config.timesteps)
                    self.backtest_config.delisted_assets[ticker] = delist_step

        # Handle randomize_universe: select subset of tickers if enabled
        if self.backtest_config.randomize_universe:
            self._select_randomized_tickers()
    ...
    def _select_randomized_tickers(self):
        """Select a random subset of tickers for Monte Carlo simulation."""
        if not self.backtest_config.randomize_universe:
            return

        # Get all available tickers from original config
        all_tickers = list(self.backtest_config.ticker_configs.keys())

        # Determine subset size
        subset_size = self.backtest_config.randomize_universe_size
        if subset_size is None or subset_size > len(all_tickers):
            subset_size = len(all_tickers)

        # Use seed for reproducibility, with counter for different selections on reset
        rng = random.Random(self.backtest_config.seed + self._randomize_counter)
        selected_tickers = rng.sample(all_tickers, subset_size)
        self._randomize_counter += 1

        # Update ticker_configs to only include selected tickers
        self.ticker_configs = {ticker: self.backtest_config.ticker_configs[ticker] for ticker in selected_tickers}

        # Update start_prices to match
        self.backtest_config.start_prices = {ticker: self.backtest_config.start_prices[ticker] for ticker in selected_tickers if ticker in self.backtest_config.start_prices}

        # Update tickers list (re-init parent class attribute)
        self.tickers = selected_tickers

    def _filter_delisted_batch(self, batch: list[TickerData], step_num: int) -> list[TickerData]:
        if not self.backtest_config.delisted_assets:
            return batch
        filtered: list[TickerData] = []
        for tick in batch:
            delist_step = self.backtest_config.delisted_assets.get(tick.ticker)
            if delist_step is None or step_num <= delist_step:
                filtered.append(tick)
        return filtered

    def _build_initial_batch(self) -> list[TickerData]:
    ...
    async def stream_price_updates(self):
        ...
        try:
            ...
            initial_batch = self._filter_delisted_batch(initial_batch, 0)
            if self._callback:
                await self._notify_callback("price_update", initial_batch)
            ...
            for step in range(max_time_steps):
                ...
                tick_batch = self._filter_delisted_batch(tick_batch, self.current_step)
    ...
    async def reset(self):
        ...
        # Reshuffle tickers if randomize_universe is enabled
        if self.backtest_config.randomize_universe:
            self._select_randomized_tickers()

        # Regenerate delisted assets if auto-generated
        if self.backtest_config.include_delisted_probability > 0:
            self.backtest_config.delisted_assets.clear()
            rng = random.Random(self.backtest_config.seed)
            for ticker in list(self.ticker_configs.keys()):
                if rng.random() < self.backtest_config.include_delisted_probability:
                    delist_step = rng.randint(1, self.backtest_config.timesteps)
    ...
    async def get_next_bar_for_client(self, client_id: str) -> BarResponse:
        ...
        try:
            ...
            async with self.step_locks[client_id]:
                ...
                bar_data = self._filter_delisted_batch(bar_data, step_num)
