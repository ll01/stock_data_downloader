...
class TestExchange(ExchangeInterface):
            ...
            try:
                if side == "BUY":
                    ...
                    if side == "SHORT":
                        ...
                        self.portfolio.sell(order.symbol, order.quantity, order.price)  # Uses sell to cover short positions
                        status = "FILLED"
                    else:
                ...
                logger.info(f"Order {order_id} {side} {order.quantity} {order.symbol} @ {order.price}: {status}")
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        if order_id in self.active_orders:
            ...
            # Notify subscribers
            await self._notify_order_update("order_canceled", self.closed_orders[order_id])
            
            logger.info(f"Order {order_id} canceled")
            return True
        else:
            logger.warning(f"Cannot cancel order {order_id}: not found or already closed")
            return False
        ...
        for sub_id, sub_info in self.subscription_callbacks.items():
            ...
            if not order_ids or order_id in order_ids:
                try:
                    ...
                    logger.error(f"Error in order update callback (sub_id {sub_id}): {e}")